...
| `DEEPSEEK_API_KEY` | ИИ: разбор анализов, документов, OCR-JSON — модель **accurate** (по умолчанию `deepseek-v4-pro`); чат и перевод — **fast** (по умолчанию `deepseek-v4-flash`). См. `DEEPSEEK_MODEL_*` в `.env.example`. |
